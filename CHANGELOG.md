...
#### v1.2 扩展功能 D4 · LLM Function Calling RAG Agent
...
- 各 LLM client 实现（含 `mock` 与 `anthropic`）均新增 `chat_with_tools()` 方法，统一返回 `ChatResponse(finish_reason, tool_calls, content)`；mock client 用规则模拟 tool_call 保证零依赖 e2e 可跑。
...
| 扩展功能数 (累计) | 11 | **17** | +6 | C3/D1/D2/C5/D7/D4 |

### v1.2.0 路线图完整完成情况

| Milestone | 扩展功能 | 状态 | tag |
| --- | --- | --- | --- |
...
- **§7.3 真实数据回填**：用 liver.h5ad PCA 30D N=30000 子集跑出 25 个数据点（5 帕累托前沿），表格内所有 `0.99XX / XXXXX` 占位全部替换为实测值；原始 JSON 保存 `docs/benchmark_data/sweep_real_liver_pca30.json`，静态 PNG `docs/assets/benchmark/pareto_pca30.png`。
...
v1.1.0 之后的 **M1 性能呈现升级**：6 个语义化 commit（feat 3 / docs 1 / chore 2 / 含 Phase 0 初始化），新增 4 个 REST 接口（3 个 sweep + 1 个 with_params）+ 2 张新表 + 1 个前端 Tab + 1 张 PPT 增量页，全部基于 `feat/v1.2-bonus` 分支。本里程碑覆盖 v1.2 路线图的 6 项扩展功能中的前 2 项（C3 帕累托曲线 / D1 交互式仪表盘）。

### 新功能 Features

#### v1.2 扩展功能 C3 · ANN-Benchmarks 风格 recall-QPS 帕累托曲线
...
第一个正式发布版本，覆盖软件工程大作业全部课程要求与三项扩展功能。
...
- **feat(backend)**: RAG 自然语言查询模块（扩展功能，规则解析 Mock + 真实 LLM 客户端）
- **feat(backend)**: 自适应 HNSW 后端（扩展功能，自适应 ef_search + 早停 + 升档）
...
- **feat(frontend)**: RAG 自然语言对话页（扩展功能前端）
