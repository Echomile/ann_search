...
单细胞 PCA 通常 dim=30~50，而蛋白 ESM2 embed=1280、文本 BERT/SBERT embed=768~1536、CLIP 视觉 embed=512 等场景维度高得多，对各 backend 的影响显著不同。本节用 N=30000 合成 `standard_normal` 在 6 个 dim 档位上做横向扫描，对应 JSON 结果 `docs/benchmark_results_dim.json`。
...
## 9. 跨数据集语义对齐：intersect_only vs 各自检索对比 (v1.2 D7 加分项)
...
| 项 | 值 |
| --- | --- |
| 源数据集 | 合成 PBMC-like：dataset A (40 细胞 / 80 基因)，dataset B (35 细胞 / 75 基因)，共享 60 基因 |
| 查询样本 | 50 条 / 数据集随机抽样 |
| Top-K | 10 |
| 对齐维度 | target_dim = 8 (受合成数据集规模限制) |
| 跨库一致性指标 | Top-K 邻居中「来自每个源数据集的占比 ≥ 1/N 的查询比例」 |
| Recall 参考真值 | 单库 brute 全扫描 (内库 Top-K) |

> 说明：单元测试与本节使用同一合成数据集 fixture (``backend/tests/test_alignment.py``)，
> 真实 PBMC 30k 数据集的回填留待 M3 演示阶段补充。

### 9.3 结果（占位 / 待 PBMC 30k 真实数据回填）

| 配置 | Recall@10 | 跨库一致性 | 备注 |
| --- | --- | --- | --- |
| 各自查 + min-max | 0.XX | 0.XX | v1.0 兼容路径，向量空间不可比 |
| intersect_only 对齐 | 0.XX | 0.XX | D7 主路径，PCA 投影后单库扫描 |
| harmony 对齐（可选） | 0.XX | 0.XX | 依赖 harmonypy，本环境未安装时降级为 intersect_only |

合成数据 fixture 上的可观测指标（M3.D7 实施时实测）：

- `intersect_only` 对齐 2 个数据集 (cells=75, genes=60) 耗时 < 2 s；
- 对齐后 ``vectors.npy`` shape = (75, 8)，``cell_map.json`` 行序保证 A 前 B 后；
- ``POST /search/multi-dataset`` 带 ``aligned_dataset_id`` 时
  ``index_backend == "aligned-brute"``，每条 hit 的 ``source_dataset_id`` ∈ 参与对齐 dataset 集合。

### 9.4 结论

- **对齐路径解锁的能力**：在统一向量空间上对所有细胞跑同一次 brute（或 ANN）检索，
  距离直接可比，跨库 Top-K 的「混库度」可观、可解释；
- **何时使用**：源数据集来自同一组织/物种（基因交集 ≥ 1000）、且需要做「跨库联合
  分析」时优先；不同物种 / 完全不同组织时基因交集塌缩，对齐意义减弱；
- **何时退回 min-max**：源数据集 schema 完全异质（PCA 维度差异大、HVG 重叠率 < 10%）、
  仅需做粗排时；min-max 仍然是最低成本的兼容路径；
- **harmonypy 加值**：在批次效应显著的同物种多 batch 场景（如 PBMC 跨实验室），
  harmony 校正后跨库一致性可进一步提升，但需付出额外 8~15s/数据集的 batch correction 成本。
