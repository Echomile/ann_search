...
> 团队规模 5 人，按"项目经理 / 后端 2 / 前端 1 / 算法 + 文档"角色组合。姓名 / 学号为占位，最终提交前由组长根据课程登记表替换。

| 序号 | 姓名 | 学号 | 角色 | 主要贡献模块 | 贡献度 |
| --- | --- | --- | --- | --- | --- |
| 1 | TBD | TBD | **组长 · 项目经理 / 后端** | 总体架构、`backend/app/api/`、`backend/app/main.py`、`Makefile`、`infra/docker-compose.yml`、`.github/workflows/`、文档统稿 | 22% |
| 2 | TBD | TBD | 后端 1（数据 & 服务）| `backend/app/services/dataset_service.py`、`backend/app/services/preprocess.py`、`backend/app/tasks/preprocess_task.py`、Alembic 迁移、单细胞数据预处理 | 20% |
| 3 | TBD | TBD | 后端 2（检索 & 评测）| `backend/app/services/search.py`、`backend/app/services/evaluation.py`、`backend/app/tasks/index_task.py`、`backend/app/tasks/benchmark_task.py`、`backend/scripts/benchmark.py` | 20% |
| 4 | TBD | TBD | 前端 / 可视化 | `frontend/src/pages/*.tsx`（8 个页面）、`frontend/src/api/`、`frontend/src/store/`、Plotly 可视化、AntD 表单与路由 | 20% |
| 5 | TBD | TBD | 算法 / 文档 / 测试 | `backend/app/services/ann/*`（5 个后端 + 工厂 + LRU）、`backend/app/services/rag.py`、`backend/tests/*` 35 个测试、`e2e/test_liver_e2e.py`、`docs/` | 18% |

### 4.1.2 RACI 矩阵

R=Responsible（执行）、A=Accountable（决策）、C=Consulted（咨询）、I=Informed（知会）。

| 子任务 / 模块 | 组长 PM | 后端 1 | 后端 2 | 前端 | 算法/文档 |
| --- | --- | --- | --- | --- | --- |
| 需求与系统设计 | **A** | C | C | C | R |
| Docker / CI / Makefile | **A/R** | C | C | I | I |
| 数据库与 Alembic | A | **R** | C | I | I |
| 数据集上传与预处理 | A | **R** | C | C | C |
| ANN 引擎（5 后端）| A | I | C | I | **R** |
| LRU Index Cache | A | I | **R** | I | C |
| 检索 API & 服务 | A | C | **R** | C | C |
| 评测脚本 & 报告 | A | I | **R** | I | C |
| 前端 8 个页面 | A | I | C | **R** | I |
| 可视化（Plotly）| A | I | C | **R** | C |
| 加分：多数据集联合 | A | I | **R** | C | C |
| 加分：Adaptive-HNSW | A | I | C | I | **R** |
| 加分：RAG | A | I | C | C | **R** |
| 单元测试 & 静态检查 | A | C | C | C | **R** |
| Playwright E2E 与截图 | A | C | C | C | **R** |
| 性能基准 & 报告 | A | I | **R** | I | C |
| 用户手册 & 开发文档 | **A** | I | I | I | **R** |
| 答辩 PPT & 演示视频 | **A/R** | C | C | C | C |
...
| M6 加分功能 | W5 | 多数据集联合、Adaptive-HNSW、RAG 对话 | 单测 + 基准报告 | ✓ 完成 |
...
- @组长：搭建 FastAPI 骨架（commit `5bf111a`）、main 路由聚合、全局异常处理；
- @后端1：完成 Auth 模块（commit `7028345`），含 JWT 签发 / 校验、Alembic 初始迁移、3 个 pytest 用例；
- @后端2：数据集 service + Scanpy 预处理（commit `5a9ebf1`），8 MB 流式上传、`Dataset.status` 状态机；
- @前端：搭建 React 18 + Vite 5 + AntD + Plotly（commit `d345451`），登录 / 注册基础页；
- @算法：审阅 ANN 抽象 API 设计、调研 FAISS / HNSWLIB 在 macOS arm64 上的安装路径。

**下周计划**

- @后端2：完成 4 后端 ANN 实现 + 工厂；
- @算法：补 8 个 ANN 后端单测；
- @前端：完成数据集列表 + 上传交互；
...
- @后端2：检索 + 评测模块完成（commit `7bfe6be`），含 post-filter / 多数据集联合 / Recall 计算；
- @算法：Adaptive-HNSW 后端 + 基准脚本（commit `d4e5d82`），实测 mean_ef=50.6；
- @算法：RAG 自然语言查询模块（commit `6bbf536`），含 mock provider、单测 7 个；
- @前端：5 个业务页面（commit `551bdfa`）：数据集 / 索引 / 检索 / 可视化 / 评测；
- @组长：修复 ruff 12 个告警（commit `909d320`）；建立 CI workflow。

**下周计划**

- @前端：RAG 对话页；
- @算法：Playwright E2E 主流程脚本；
- @组长：性能基准报告。
...
- @前端：RAG 对话页完成（commit `1c5afb1`）；
- @组长：修复 CORS_ORIGINS 解析 bug（commit `af8a53e`）；
- @前端：修复登录字段对齐（commit `8a2b94b`）；
- @前端：修复 multipart 上传（commit `7d88ef1`）；
- @算法：Playwright E2E（commit `643cbcf`），跑通 1.3 GB liver.h5ad 全流程，归档 9 张截图；
- @全员：文档统稿（5 篇 + API 速查表）。

**下周计划**

- @全员：录制演示视频、制作答辩 PPT；
- @组长：组织 dry-run，校准答辩节奏。
